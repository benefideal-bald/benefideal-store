...
db.serialize(() => {
    ...
    db.get(`SELECT COUNT(*) as count FROM reviews`, (err, row) => {
        ...
        if (row && row.count === 0) {
            const staticReviews = [
                // Максим и Тимур - НОВЕЙШИЕ отзывы (сегодня и вчера) - должны быть первыми!
                { name: 'Максим', email: 'static_review_maxim@benefideal.com', text: 'Приобрел кепкат про на месяц, все работает как следует', rating: 4, order_id: 'STATIC_REVIEW_MAXIM', daysAgo: 0 }, // Сегодня (новейший)
            ...
            staticReviews.forEach((review) => {
                ...
                // Устанавливаем время для Максима - сегодня 15:42, для Тимура - вчера 13:57
