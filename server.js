...
const db = new sqlite3.Database(dbPath, sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE, (err) => {
    if (err) {
        ...
        // КРИТИЧЕСКИ ВАЖНО: Проверяем, что все отзывы из reviews.json закоммичены в Git
        // Это защита от потери отзывов при деплое
        console.log(`🔍 Проверка защиты отзывов: проверяем, что все отзывы в Git...`);
        if (fs.existsSync(reviewsJsonPathGit)) {
            try {
                const localReviews = JSON.parse(fs.readFileSync(reviewsJsonPathGit, 'utf8'));
                console.log(`   📋 Найдено ${localReviews.length} отзывов в локальном reviews.json`);
                
                // Пытаемся закоммитить в Git (если есть изменения)
                // Это гарантирует, что даже если предыдущий коммит не успел, отзывы не потеряются
                commitReviewsToGitViaAPI().then(success => {
                    if (success) {
                        console.log(`✅✅✅ Все отзывы защищены - закоммичены в Git при старте сервера!`);
                    } else {
                        console.warn(`⚠️ Не удалось закоммитить отзывы в Git при старте (возможно, уже закоммичены или нет GITHUB_TOKEN)`);
                    }
                }).catch(err => {
                    console.warn(`⚠️ Ошибка при проверке коммита отзывов: ${err.message}`);
                });
            } catch (error) {
