...
db.serialize(() => {
    ...
    db.get(`SELECT COUNT(*) as count FROM reviews`, (err, row) => {
        ...
        if (row && row.count === 0) {
            const staticReviews = [
                // Максим и Тимур - статические отзывы (вчера и позавчера) - не должны перекрывать новые клиентские отзывы
                { name: 'Максим', email: 'static_review_maxim@benefideal.com', text: 'Приобрел кепкат про на месяц, все работает как следует', rating: 4, order_id: 'STATIC_REVIEW_MAXIM', daysAgo: 1 }, // Вчера (чтобы новые отзывы были первыми)
                { name: 'Тимур', email: 'static_review_timur@benefideal.com', text: 'Купил чат гпт на месяц, сделали все быстро, рекомендую 🫡', rating: 5, order_id: 'STATIC_REVIEW_TIMUR', daysAgo: 2 }, // Позавчера
